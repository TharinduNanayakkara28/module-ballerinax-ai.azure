...
public isolated client class OpenAiModelProvider {
    *ai:ModelProvider;
    ...
    # + temperature - The temperature for controlling randomness in the model's output. Leave unset (`()`) for
    #                 reasoning ("thinking") model deployments (e.g. `o1`, `o3`, `o4-mini`); these reject any
    #                 explicit temperature other than their default
    ...
    public isolated function init(@display {label: "Service URL"} string serviceUrl,
        ...
        if streamClient is error {
            return error ai:Error("Failed to initialize the Azure OpenAI streaming client", streamClient);
        }
    ...
    remote function chatStream(ai:ChatMessage[]|ai:ChatUserMessage messages,
        ...
        chat:ChatCompletionRequestMessage[] requestMessages = check self.mapToChatCompletionRequestMessage(messages);
        map<json> request = {
            messages: requestMessages.toJson(),
            max_tokens: self.maxTokens,
            'stream: true,
...
# Iterator that converts Azure OpenAI's Server-Sent Event stream into a stream of
# normalized `ai:ChatCompletionChunk` values. Each `data:` line is parsed into the
# Azure wire chunk and mapped via `toAiChunk`; the terminating `[DONE]` sentinel,
